...
// console.log("rammmmmmmmmmmmmmmmmmmmmmmmm",)
...
//     }
//     return {
//       invoiceType: route?.params?.isRepair ? "repair" : "order",
//       selectedProduct: [],
//       photo: null,
...
const ORInvoiceForm = ({ title, navigation, route, invoiceHandler }) => {
  ...
  // Log editData to debug
  console.log("ORInvoiceForm editData:", JSON.stringify(editData, null, 2));

  // Initialize invoices state with type from route or editData
  const [invoices, setInvoices] = useState(() => {
    if (editData && isEditing) {
      // If selectedProduct exists, use its first item's sale data for top-level fields
      const saleData = editData.selectedProduct?.[0]?.sale || {};
      const showInBill = editData.showInBill || {
        weight: false,
        less_weight: false,
        net_weight: false,
        tounch: false,
      ...
      const initialState = {
        invoiceType: editData.invoiceType || "repair",
        client: editData.client ? {
          value: editData.client.id,
          label: editData.client.name,
          role_id: editData.client.role_id
        } : null,
        selectedProduct: editData.selectedProduct || [],
        photo: editData.photo || null,
        additional_photo: editData.additional_photo || null,
        huid: editData.huid || saleData.huid || "",
        hsn_id: editData.hsn_id ? {
          value: editData.hsn_id.value || editData.hsn_id.id,
          label: editData.hsn_id.label || editData.hsn_id.name,
          variation: editData.hsn_id.variation_data || []
        } : null,
        payment_mode: editData.payment_mode || saleData.payment_mode || "Cash",
        amount_paid: (editData.amount_paid || saleData.amount_paid || 0).toString(),
        totalPrice: (editData.totalPrice || saleData.net_price || 0).toString(),
        weight: (editData.weight || saleData.gross_weight || 0).toString(),
        less_weight: (editData.less_weight || saleData.less_weight || 0).toString(),
        net_weight: (editData.net_weight || saleData.net_weight || 0).toString(),
        tounch: (editData.tounch || saleData.tounch || 0).toString(),
        wastage: (editData.wastage || saleData.wastage || 0).toString(),
        fine_weight: (editData.fine_weight || saleData.fine_weight || 0).toString(),
        rate: editData.rate || saleData.rate || "0",
        metal_value: (editData.metal_value || saleData.metal_value || 0).toString(),
        making_type: editData.making_type || saleData.making_type || { value: "PG", label: "Per Gram" },
        making_charge: (editData.making_charge || saleData.making_charge || 0).toString(),
        making_amount: (editData.making_amount || saleData.making_amount || 0).toString(),
        polishing: (editData.polishing || saleData.polishing || 0).toString(),
        stone_setting: (editData.stone_setting || saleData.stone_setting || 0).toString(),
        additional_charges: (editData.additional_charges || saleData.additional_charges || 0).toString(),
        subtotal: (editData.subtotal || saleData.subtotal || 0).toString(),
        tax: (editData.tax || saleData.tax || 3).toString(),
        tax_amount: (editData.tax_amount || saleData.tax_amount || 0).toString(),
        final_amount: (editData.final_amount || saleData.net_price || 0).toString(),
        balance: (editData.balance || (parseFloat(editData.final_amount || saleData.net_price || 0) - parseFloat(editData.amount_paid || 0))).toString(),
        remark: editData.remark || saleData.remark || "",
        comment: editData.comment || saleData.comment || "",
        size: editData.size || saleData.size || "",
        showSizeInBill: editData.showSizeInBill || saleData.showSizeInBill || false,
        showCommentInBill: editData.showCommentInBill || saleData.showCommentInBill || false,
        given_weight: (editData.given_weight || saleData.given_weight || 0).toString(),
        received_weight: (editData.received_weight || saleData.received_weight || 0).toString(),
        cutting_enabled: editData.cutting_enabled || saleData.cutting_enabled || false,
        showInBill: {
          weight: showInBill.weight || false,
          less_weight: showInBill.less_weight || false,
          net_weight: showInBill.net_weight || false,
          tounch: showInBill.tounch || false,
          wastage: showInBill.wastage || false,
          fine_weight: showInBill.fine_weight || false,
          rate: showInBill.rate || false,
          making_charge: showInBill.making_charge || false,
          making_amount: showInBill.making_amount || false,
          tax: showInBill.tax || false,
          tax_amount: showInBill.tax_amount || false,
          final_amount: showInBill.final_amount || false,
          huid: showInBill.huid || false,
          hsn: showInBill.hsn || false,
          polishing: showInBill.polishing || false,
          stone_setting: showInBill.stone_setting || false,
          subtotal: showInBill.subtotal || false,
        },
      };

      // Log initial invoices state for debugging
      console.log("Initial invoices state:", JSON.stringify(initialState, null, 2));
      return initialState;
    }
    return {
      invoiceType: route?.params?.isRepair ? "repair" : "order",
  ...
  useEffect(() => {
    if (editData?.client && groups.length > 0 && !role) {
      // Set role based on client data
      const clientRole = groups.find(g => g.id === editData.client.role_id);
      if (clientRole) {
        setRole({

...
import FilterByName from '@/src/components/common/FilterByName';
import { ApiRequest } from "@/src/utils/api";
import { MANAGE_GROUPS_API, OR_INVOICE_API, BASE_URL } from "../../../utils/api"; // Use absolute path
import Toast from "react-native-toast-message";

console.log('Imported OR_INVOICE_API:', OR_INVOICE_API); // Debug import
...
const USE_MOCK_DATA = true; // Keep true until import issue is resolved
...
const Allocation = ({ navigation }) => {
  ...
  const [assignModalVisible, setAssignModalVisible] = useState(false);
  const [selectedItems, setSelectedItems] = useState([]);
  const [availableItems, setAvailableItems] = useState(USE_MOCK_DATA ? MOCK_ITEMS : []);
  const [loading, setLoading] = useState(false);
  const [itemTypeFilter, setItemTypeFilter] = useState('ALL');
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  ...
  useEffect(() => {
    console.log('OR_INVOICE_API in useEffect:', OR_INVOICE_API);
    fetchGroups();
    if (!USE_MOCK_DATA) {
      fetchMembers();
      fetchAvailableItems();
  ...
  const fetchMembers = async (page = currentPage, search = searchTerm) => {
    ...
    try {
      setLoading(true);
      const response = await ApiRequest({
        url: `${BASE_URL}${MANAGE_GROUPS_API.membersList}`,
        method: 'GET',
        params: {
          page,
          search,
  ...
  const fetchAvailableItems = async () => {
    ...
    try {
      if (!OR_INVOICE_API?.listAll) {
        throw new Error('OR_INVOICE_API.listAll is undefined. Check API configuration in @/src/utils/api.js.');
      }

      setLoading(true);
      const [ordersResponse, repairsResponse] = await Promise.all([
        ApiRequest({
          url: `${BASE_URL}${OR_INVOICE_API.listAll}`,
          method: 'GET',
          params: {
            status: 'PENDING',
            type: 'order'
      ...
      if (repairsResponse.success) {
        console.log('Repairs response:', repairsResponse.data);
        const repairItems = repairsResponse.data
          .filter(repair => !repair.assigned_to)
          .flatMap(repair => repair.selectedProduct?.map(product => ({
            id: product.id,
            type: 'REPAIR',
            order_number: repair.id,
      ...
      console.error('Failed to fetch items:', error);
      ShowToast(error.message || 'Failed to fetch available items', 'error');
      setAvailableItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAssignItems = async () => {
    ...
    try {
      setLoading(true);
      const response = await ApiRequest({
        url: `${BASE_URL}${MANAGE_GROUPS_API.assignItems}`,
        method: 'POST',
        data: {
          member_id: selectedMember.id,
          items: selectedItems.map(item => ({
        ...
        <View className="bg-white w-11/12 rounded-xl p-4 max-h-[80%]">
          ...
          <ScrollView className="max-h-96">
            {filteredItems.length > 0 ? (
              ...
              <Text className="text-gray-500 text-center py-4">No available items</Text>
            )}
          </ScrollView>
...
// import FilterByName from '@/src/components/common/FilterByName';
// import { ApiRequest } from "@/src/utils/api";
// import { MANAGE_GROUPS_API, OR_INVOICE_API } from "../../../utils/api/endpoints.js"; // Try "@/src/utils/api/endpoints" if needed
// import ShowToast from "@/src/components/common/ShowToast";

// // Debugging imports
// console.log('Imported MANAGE_GROUPS_APIramaaaaa:', MANAGE_GROUPS_API);
// console.log('Imported OR_INVOICE_APIaaaaaaaaaaa:', OR_INVOICE_API);
...
//   // Fallback UI for missing API configurations
//   if (!MANAGE_GROUPS_API?.membersList || !OR_INVOICE_API?.listAll) {
//     return (
...
//         </Text>
//         <Text className="text-gray-500 text-center">
//           Unable to load allocation data due to missing API configurations.
//         </Text>
//       </View>
//     );
//   }
...
//     } catch (error) {
//       console.error('Failed to fetch items:', error);
//       ShowToast(error.message || 'Failed to fetch items');
//     }
//   };
